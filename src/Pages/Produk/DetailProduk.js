...
function InputForm(props){
  ...
  async function onSubmit(event){
    ...
    axios.put(`http://localhost:8000/product/${inputData.id}`,{
      name: inputData.name,
  ...
  return(
    <Grid container direction="column" component={Paper} className={classes.boxInput}>
      ...
      <Box display="flex" style={{marginBlock:10}}>
        <TextField onChange={(event)=>{setInputData({...inputData, category: event.target.value}); event.preventDefault()}} value={inputData.category} label="Kategori" variant="outlined" size="small" style={{width:450}}/>
      </Box>
      <Box display="flex" style={{marginBlock:10}}>
        <TextField onChange={(event)=>{setInputData({...inputData, recommendation_price: event.target.value}); event.preventDefault()}} value={inputData.recommendation_price} label="Harga Rekomendasi" variant="outlined" size="small" style={{width:450}}/>
      </Box>
      <Box display="flex" style={{marginBlock:10}}>
...
function DetailProduk(props) {
    ...
    return(
      <Box>
        <Typography variant="h5" color="primary" style={{marginBottom:20}}>Detail Barang</Typography>
        <Button onClick={(event)=>handleChangeUpdate(event)} variant="outlined" color={updateState === 1 ? 'secondary':'primary'}>{updateState ? 'Cancel Update':'Update Barang'}</Button>
        {
            updateState === 1 ? <InputForm produkData={master}/> : <DisplayDetail produkData={master}/>
        }
        {/* <InputForm/> */}
      </Box>
    )
